...
def print_error(error_msg, data):
    ...
    if 'Shooting failed' in error_msg:
      err_code = 0
    if all(x in error_msg for x in ['Gradient', 'scalar']):
      err_code = 1
    if all(x in error_msg for x in ['Gradient', 'tensor']):
      err_code = 2
    if all(x in error_msg for x in ['Ghost', 'scalar']):
      err_code = 3
    if all(x in error_msg for x in ['Ghost', 'tensor']):
      err_code = 4
